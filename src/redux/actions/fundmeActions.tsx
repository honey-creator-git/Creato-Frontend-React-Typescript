...
export const fundmeAction = {
  ...
  saveFundme: (fundme: any, teaser: any, cover: any, navigate: any, url: any) => async (dispatch: Dispatch<any>) => {
    ...
    if (fundme.title === null && fundme.deadline === null && fundme.category === null && (fundme.teaser === null && teaser === null)
      && (fundme.options.length === 0 || (fundme.options.length !== 0 && fundme.options[0].option.title === "" && fundme.options[1].option.title === ""))) {
      navigate(url);
    } else {
  ...
  checkSupportAndResults: (fundmeId: any, optionId: any, navigate: any) => async (dispatch: Dispatch<any>) => {
    api.checkFundMeFinished(fundmeId)
      .then((result: any) => {
        const { data } = result;
        if (data.finished) navigate(`/fundme/result/${fundmeId}`);
        else navigate(`/fundme/${fundmeId}/support/${optionId}`);
      }).catch((err: any) => console.log(err));
  },
